...
const PluginItemInput: React.FC<ItemProps> = props => {
    ...
    return (
        <span>
            <img src={icon} />
            <InputNumber min={0} defaultValue={0} step={1} onChange={c} />
        </span>
    );
};
...
const PluginInput: React.FC<PluginInputProps> = props => {
    ...
    return <span >
        <PluginItemInput icon={speed.getIconUrl()} onChange={changeSpeedNum} name={'3级速度插件'}/>
        <PluginItemInput icon={product.getIconUrl()} onChange={changeSpeedNum} name={'3级产能插件'}/>
    </span>;
};
